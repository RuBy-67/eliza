...
export class AgentRuntime implements IAgentRuntime {
    ...
    constructor(opts: {
        ...
        // Appeler la méthode asynchrone pour initialiser le caractère
        this.initCharacter(opts.character);

        // On continue l'initialisation après avoir défini le character
        ...
        // Crée la room et l'utilisateur (on appelle des méthodes asynchrones)
        ...
        // Suite de l'initialisation des autres composants
        this.initManagers(opts);
        this.initModelProviders();

        elizaLogger.success(`Agent ID: ${this.agentId}`);
    }

    private async initCharacter(character?: Character | Promise<Character>) {
        const characterToUse =
            character instanceof Promise ? await character : character;

        // Assignation de l'agentId et du character
        this.agentId =
            characterToUse?.id ??
            stringToUuid(characterToUse?.name ?? uuidv4());
        this.character = characterToUse;

        // Log avec characterToUse
        elizaLogger.info(
    ...
    private initModelProviders() {
        elizaLogger.info(
            `${this.character.name}(${this.agentId}) - Setting Model Provider:`,
            {
                characterModelProvider: this.character.modelProvider,
                optsModelProvider: this.modelProvider,
